...
export class BrowserMachine {
  ...
  private constructor(mmu: MMU) {
    ...
    const cgbHook = {
      readIO: (addr: number) => this.mmu.readCgbIo(addr),
      writeIO: (addr: number, value: number) => {
        if ((addr >= 0xff51 && addr <= 0xff55) || [0xff4d,0xff4f,0xff68,0xff69,0xff6a,0xff6b,0xff70].includes(addr)) {
          (this.mmu as any).handleCgbIoWrite(addr, value & 0xff);
          return true;
        }
        return false;
      }
    ...
    const chain = {
      readIO: (addr: number) => { const v = this.joypad.readIO(addr); if (v !== null) return v; return cgbHook.readIO(addr); },
      writeIO: (addr: number, value: number) => this.joypad.writeIO(addr, value) || cgbHook.writeIO(addr, value),
    };
  ...
  static async create(wasmBytes: Uint8Array, romBytes: Uint8Array): Promise<BrowserMachine> {
    ...
    if (mmu.isCgb()) { exportsRef.set_A(0x11); exportsRef.set_F(0x80); exportsRef.set_B(0x00); exportsRef.set_C(0x00); }
    else { exportsRef.set_A(0x01); exportsRef.set_F(0xb0); exportsRef.set_B(0x00); exportsRef.set_C(0x13); }
    exportsRef.set_D(0x00); exportsRef.set_E(mmu.isCgb() ? 0x08 : 0xd8);
