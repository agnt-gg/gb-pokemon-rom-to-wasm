...
import {
  ...
  exportSaveStateBundle, importSaveStateBundle,
  downloadBytes, downloadJson, pickFile,
} from "./save.ts";
...
async function boot() {
  ...
  document.getElementById("btn-export-sav")?.addEventListener("click", async () => {
    ...
    if (!data.length) { toast("No battery SRAM to export"); return; }
    downloadBytes(name, data); toast("Exported " + name + " (" + (data.length / 1024).toFixed(0) + "KB)");
  });
  document.getElementById("btn-import-sav")?.addEventListener("click", async () => {
    ...
    if (!data) return;
    machine.loadBatterySave(data); await saver.flush(); toast("Imported .sav (" + (data.length / 1024).toFixed(0) + "KB)");
  });
  document.getElementById("btn-export-states")?.addEventListener("click", () => {
    ...
    if (!bundle.slots.length) { toast("No save states to export"); return; }
    const name = (machine.saveKey().split(":")[1] || "game") + ".states.json";
    downloadJson(name, bundle); toast("Exported " + bundle.slots.length + " save state(s)");
  });
  document.getElementById("btn-import-states")?.addEventListener("click", async () => {
    ...
    try {
      ...
      refreshSlots(); toast("Imported " + n + " save state(s)");
    } catch (e) { toast("Save-state import failed"); console.warn(e); }
  });
