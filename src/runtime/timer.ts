...
export class Timer {
  ...
  serializeState(): { divCounter: number; timaCounter: number } {
    return { divCounter: this.divCounter, timaCounter: this.timaCounter };
  }
  loadState(s: { divCounter: number; timaCounter: number }): void {
    this.divCounter = s.divCounter; this.timaCounter = s.timaCounter;
  }
