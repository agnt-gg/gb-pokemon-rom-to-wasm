...
export class MMU {
    ...
    extRamEnabled = false;
    // MBC state
    romBank = 1; // bank mapped at 0x4000-0x7FFF (never 0 for the switchable window)
    ramBank = 0;
    bankingMode = 0; // MBC1 mode select
    ...
    write(addr, value) {
        ...
        if (addr < 0xff80) {
            if (this.hooks && this.hooks.writeIO(addr, value))
                return;
    ...
    rawIoWrite(addr, v) { this.io[addr - 0xff00] = v & 0xff; }
    getExtRam() { return this.extRam; }
    loadExtRam(data) { this.extRam.set(data.subarray(0, this.extRam.length)); }
    currentRtcRegs() {
    ...
    loadState(s) {
        ...
        this.bankingMode = s.bankingMode;
        this.extRamEnabled = s.extRamEnabled;
        if (s.rtcBaseUnixMs !== undefined)
            this.rtcBaseUnixMs = s.rtcBaseUnixMs;
        if (s.rtcLatched !== undefined)
            this.rtcLatched = s.rtcLatched;
        if (s.rtcLatchLast !== undefined)
            this.rtcLatchLast = s.rtcLatchLast;
        if (s.rtcLatchedRegs)
