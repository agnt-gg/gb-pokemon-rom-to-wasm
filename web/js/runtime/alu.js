...
export function adc8(a, b, carryIn) {
    const c = carryIn ? 1 : 0;
    const r = a + b + c;
    const v = r & 0xff;
...
export function sbc8(a, b, carryIn) {
    const c = carryIn ? 1 : 0;
    const r = a - b - c;
    const v = r & 0xff;
...
export function add16(hl, rr, fIn) {
    const r = hl + rr;
    const v = r & 0xffff;
...
export function inc16(rr) {
    return (rr + 1) & 0xffff;
}
export function dec16(rr) {
    return (rr - 1) & 0xffff;
}
// ---------------------------------------------------------------------------
// Rotates & shifts (CB-prefixed + accumulator forms)
// ---------------------------------------------------------------------------
export function rlc(v) {
    const c = (v >> 7) & 1;
    const r = ((v << 1) | c) & 0xff;
    return { v: r, f: flags(r === 0, false, false, c === 1) };
}
export function rrc(v) {
    const c = v & 1;
    const r = ((v >> 1) | (c << 7)) & 0xff;
    return { v: r, f: flags(r === 0, false, false, c === 1) };
}
export function rl(v, carryIn) {
    const c = (v >> 7) & 1;
    const r = ((v << 1) | (carryIn ? 1 : 0)) & 0xff;
    return { v: r, f: flags(r === 0, false, false, c === 1) };
}
export function rr(v, carryIn) {
    const c = v & 1;
    const r = ((v >> 1) | (carryIn ? 0x80 : 0)) & 0xff;
    return { v: r, f: flags(r === 0, false, false, c === 1) };
}
export function sla(v) {
    const c = (v >> 7) & 1;
    const r = (v << 1) & 0xff;
    return { v: r, f: flags(r === 0, false, false, c === 1) };
}
export function sra(v) {
    const c = v & 1;
    const r = ((v >> 1) | (v & 0x80)) & 0xff; // arithmetic: keep sign bit
    return { v: r, f: flags(r === 0, false, false, c === 1) };
}
export function srl(v) {
    const c = v & 1;
    const r = (v >> 1) & 0xff;
    return { v: r, f: flags(r === 0, false, false, c === 1) };
}
export function swap(v) {
 ...
 */
export function rlca(a) {
    const r = rlc(a);
    return { v: r.v, f: r.f & ~FLAG_Z };
}
export function rrca(a) {
    const r = rrc(a);
    return { v: r.v, f: r.f & ~FLAG_Z };
}
export function rla(a, carryIn) {
    const r = rl(a, carryIn);
    return { v: r.v, f: r.f & ~FLAG_Z };
}
export function rra(a, carryIn) {
...
export function bit(n, v, fIn) {
    const set = (v >> n) & 1;
    return (set ? 0 : FLAG_Z) | FLAG_H | (fIn & FLAG_C);
}
export function res(n, v) {
    return v & ~(1 << n) & 0xff;
}
