...
import { liftInstr, blockName, SENTINEL_HALT } from "./lifter.js";
class Emit {
    lines = [];
    ...
    while (count < MAX) {
        ...
        if (term) {
            for (const t of ins.staticTargets ?? []) {
                if (t < 0x8000)
                    successors.push(t); // only ROM-resident targets are statically liftable
            }
            break;
        }
 ...
 */
export function buildBlocks(mem, entries, opts = {}) {
    ...
    while (queue.length && blocks.size < maxBlocks) {
        ...
        if (addr >= 0x8000)
            continue; // can't statically lift RAM
        if (blocks.has(addr))
            continue;
        const blk = liftBlock(mem, addr);
        blocks.set(addr, blk);
        for (const s of blk.successors) {
            if (!blocks.has(s) && s < 0x8000)
                queue.push(s);
        }
    }
    return blocks;
...
export function buildModuleWat(blocks) {
    const e = new Emit();
    e.push(`(module`);
