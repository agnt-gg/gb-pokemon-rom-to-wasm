...
const NATIVE_LD_SHAPES = new Set(["reg8<-reg8", "reg8<-mem_reg16", "reg8<-mem_imm16", "reg8<-mem_high_imm8", "reg8<-mem_high_c", "mem_reg16<-reg8", "mem_imm16<-reg8", "mem_high_imm8<-reg8"]);
function ldShape(ins) {
    const d = ins.operands[0], s = ins.operands[ins.operands.length - 1];
    return d && s ? `${d.kind}<-${s.kind}` : "";
}
function aluSrcShape(ins) {
    const s = ins.operands[ins.operands.length - 1];
    return s?.kind ?? "";
}
function canNativeNonTerminator(ins) {
    if (ins.mnemonic === "LD")
        return NATIVE_LD_SHAPES.has(ldShape(ins));
    if (ins.mnemonic === "INC" || ins.mnemonic === "DEC")
        return ins.operands[0]?.kind === "reg8";
    if (ins.mnemonic === "AND" || ins.mnemonic === "OR" || ins.mnemonic === "CP" || ins.mnemonic === "ADD" || ins.mnemonic === "SUB") {
        return ins.operands[0]?.kind === "reg8" && ins.operands[0].reg === "A" && ["reg8", "imm8"].includes(aluSrcShape(ins));
    }
    return NATIVE_NON_TERMINATORS.has(ins.mnemonic);
}
/** Emit a label name for a block at a given address. */
export function blockName(addr) {
 ...
 */
export function liftInstr(e, ins) {
    ...
    switch (ins.mnemonic) {
        ...
        case "JP": {
            ...
            if (!cond) {
                ...
                emitConditionalReturn(e, cond.cc, tgt, (ins.addr + ins.length) & 0xffff, ins);
            }
            return true;
        }
        case "JR": {
            ...
            if (!cond) {
                ...
                emitConditionalReturn(e, cond.cc, tgt, next, ins);
            }
            return true;
        }
        case "CALL": {
        ...
        case "DI": {
            e.push(`    (call $set_ime (i32.const 0))`);
            irqAfter();
            return false;
        }
        case "EI": {
            e.push(`    (call $sched_ei)`);
            irqAfter();
            return false;
        }
        case "ILLEGAL": {
...
function emitConditionalReturn(e, cc, tgt, next, ins) {
    const extra = Math.max(0, (ins.cyclesBranch ?? ins.cycles) - ins.cycles);
