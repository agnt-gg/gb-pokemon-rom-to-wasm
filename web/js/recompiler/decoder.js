 ...
 */
import { BASE, CB, ILLEGAL_OPCODES } from "./opcodes.js";
const REG8 = new Set(["A", "B", "C", "D", "E", "H", "L", "F"]);
const REG16 = new Set(["AF", "BC", "DE", "HL", "SP", "PC"]);
const CONDS = new Set(["NZ", "Z", "NC", "C"]);
/** read little-endian u16 */
function rd16(buf, off) {
 ...
 */
function parseOperand(tok, buf, immOff, allowCond) {
    tok = tok.trim();
    if (tok === "")
        return null;
    ...
    if (allowCond && CONDS.has(tok))
        return { kind: "cond", cc: tok };
    // immediates
    if (tok === "d8")
        return { kind: "imm8", value: buf[immOff] };
    if (tok === "d16" || tok === "a16")
        return { kind: "imm16", value: rd16(buf, immOff) };
    if (tok === "r8")
        return { kind: "simm8", value: s8(buf[immOff]) };
    if (tok === "a8")
        return { kind: "imm8", value: buf[immOff] };
    // special pointer/offset forms
    if (tok === "(a16)")
        return { kind: "mem_imm16", addr: rd16(buf, immOff) };
    if (tok === "(a8)")
        return { kind: "mem_high_imm8", off: buf[immOff] };
    if (tok === "(C)")
        return { kind: "mem_high_c" };
    if (tok === "(HL+)")
        return { kind: "mem_hl_inc" };
    if (tok === "(HL-)")
        return { kind: "mem_hl_dec" };
    if (tok === "(BC)")
        return { kind: "mem_reg16", reg: "BC" };
    if (tok === "(DE)")
        return { kind: "mem_reg16", reg: "DE" };
    if (tok === "(HL)")
        return { kind: "mem_reg16", reg: "HL" };
    ...
    if (/^[0-7]$/.test(tok))
        return { kind: "bit", n: parseInt(tok, 10) };
    // registers
    if (REG16.has(tok))
        return { kind: "reg16", reg: tok };
    if (REG8.has(tok))
        return { kind: "reg8", reg: tok };
    return null;
}
/** Map a template head word to a canonical Mnemonic, accounting for 16-bit ALU variants. */
function resolveMnemonic(head, def) {
    // 16-bit add: "ADD HL, ..." or "ADD SP, r8" stay as ADD/ADD16 distinction
    if (head === "ADD") {
        if (def.m.startsWith("ADD HL"))
            return "ADD16";
        return "ADD"; // ADD A,.. and ADD SP,r8 (lifter checks operands)
    }
    if (head === "INC")
        return def.m.match(/INC (BC|DE|HL|SP)/) ? "INC16" : "INC";
    if (head === "DEC")
        return def.m.match(/DEC (BC|DE|HL|SP)/) ? "DEC16" : "DEC";
    return head;
}
/**
 ...
 */
export function decode(buf, off, baseAddr) {
...
function expand(def, buf, off, baseAddr, opcode, cbByte) {
    ...
    const { isTerminator, staticTargets } = classifyFlow(mnemonic, operands, baseAddr, def.len);
    return {
        addr: baseAddr,
        opcode,
        cbByte,
...
function classifyFlow(m, operands, baseAddr, len) {
    ...
    switch (m) {
        case "JP": {
            ...
            if (!imm)
                return { isTerminator: true }; // JP (HL)
 ...
 */
export function disassembleRange(buf, start, end, baseAddr) {
    const out = [];
    let off = start;
    let addr = baseAddr;
    while (off < end) {
