...
import { BatteryAutoSaver } from "./save.js";
...
async function boot() {
    ...
    scene.add(key);
    const rimPink = new THREE.PointLight(0xe53d8f, 70, 35);
    rimPink.position.set(-7, 2, -4);
    scene.add(rimPink);
    const rimCyan = new THREE.PointLight(0x12e0ff, 55, 35);
    rimCyan.position.set(7, -1, -5);
    scene.add(rimCyan);
    const ground = new THREE.Mesh(new THREE.CircleGeometry(50, 64), new THREE.MeshStandardMaterial({ color: 0x0a0b12, roughness: 0.85, metalness: 0.2 }));
    ...
    armV.position.copy(dpadOrigin);
    armH.position.copy(dpadOrigin);
    armV.castShadow = armH.castShadow = true;
    gb.add(armV);
    gb.add(armH);
    const mkPad = (dx, dy, b) => {
        const m = new THREE.Mesh(new THREE.BoxGeometry(u * 0.065, u * 0.065, u * 0.05), new THREE.MeshBasicMaterial({ transparent: true, opacity: 0 }));
        m.position.set(dpadOrigin.x + dx, dpadOrigin.y + dy, dpadOrigin.z);
        addPressable(m, b);
    };
    mkPad(0, u * 0.065, "up");
    mkPad(0, -u * 0.065, "down");
    mkPad(-u * 0.065, 0, "left");
    mkPad(u * 0.065, 0, "right");
    // A / B (lower-right, red)
    ...
    const mkPill = (x, b) => {
        ...
        m.rotation.x = Math.PI / 2;
        m.position.set(x, -size.y * 0.32, btnDepth);
        m.castShadow = true;
        addPressable(m, b);
    };
    mkPill(-u * 0.08, "select");
    mkPill(u * 0.08, "start");
    // ---------------------------------------------------------------- INPUT
    const held = new Set();
    const press = (b, down) => { machine.setButton(b, down); down ? held.add(b) : held.delete(b); };
    window.addEventListener("keydown", (e) => { const b = KEYMAP[e.code]; if (b) {
        press(b, true);
        e.preventDefault();
    } });
    window.addEventListener("keyup", (e) => { const b = KEYMAP[e.code]; if (b) {
        press(b, false);
        e.preventDefault();
    } });
    ...
    const releasePtr = () => { if (activePtr) {
        press(activePtr, false);
        activePtr = null;
    ...
    function loop(now) {
        ...
        acc += dt;
        if (fc % 20 === 0) {
            fpsEl.textContent = (1000 / (acc / 20)).toFixed(0) + " fps";
            acc = 0;
        }
        saver.tick();
        controls.update();
