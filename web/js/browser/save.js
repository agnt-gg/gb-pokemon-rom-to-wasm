...
export async function idbPut(key, data) {
...
// ---------------------------------------------------------------------------
function slotKey(saveKey, slot) {
    return `${saveKey}:state:${slot}`;
}
function autoCheckpointKey(saveKey) {
    return `${saveKey}:state:auto-checkpoint`;
}
export function writeAutoCheckpoint(saveKey, state) {
    localStorage.setItem(autoCheckpointKey(saveKey), JSON.stringify({ savedAt: Date.now(), state }));
}
export function readAutoCheckpoint(saveKey) {
    const raw = localStorage.getItem(autoCheckpointKey(saveKey));
    return raw ? JSON.parse(raw) : null;
}
export function writeSaveState(saveKey, slot, state) {
    try {
...
export function readSaveState(saveKey, slot) {
    const raw = localStorage.getItem(slotKey(saveKey, slot));
    return raw ? JSON.parse(raw) : null;
}
export function listSaveStates(saveKey, slots) {
    const out = [];
    for (let i = 0; i < slots; i++) {
        const s = readSaveState(saveKey, i);
        if (s)
            out.push({ slot: i, frames: s.frames });
    }
    return out;
}
export function exportSaveStateBundle(saveKey, slots) {
    const out = { kind: "gb-recomp-save-states", version: 1, saveKey, exportedAt: Date.now(), slots: [] };
    for (let i = 0; i < slots; i++) {
        const state = readSaveState(saveKey, i);
        if (state)
            out.slots.push({ slot: i, state });
    }
    return out;
}
export function importSaveStateBundle(saveKey, bundle) {
    ...
    for (const row of bundle.slots) {
        if (!row || typeof row.slot !== "number" || !row.state)
            continue;
...
export class BatteryAutoSaver {
    ...
    async loadAtBoot() {
        if (!this.machine.hasBattery())
            return false;
        const data = await idbGet(this.key);
        if (data && data.length) {
            this.machine.loadBatterySave(data);
            return true;
        }
        return false;
    }
