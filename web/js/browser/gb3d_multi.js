 ...
 */
import { BrowserMachine } from "./host_browser.js";
import { BatteryAutoSaver, writeSaveState, readSaveState, listSaveStates, writeAutoCheckpoint, readAutoCheckpoint, exportSaveStateBundle, importSaveStateBundle, downloadBytes, downloadJson, pickFile, } from "./save.js";
import { AudioSink } from "./audio.js";
...
const normalizeTheme = (t) => t === "dmg" ? "grass" : (t === "charizard" ? "fire" : (t || "midnight"));
let PAL = THEMES[normalizeTheme(localStorage.getItem("gb-theme"))] || THEMES.midnight;
...
async function boot() {
    ...
    function renderRomPicker(catalog) {
        ...
        for (const item of catalog.items || []) {
            ...
            btn.addEventListener("click", () => {
                if (item.id === activeRomId)
                    return;
                localStorage.setItem("gb-3d-library-rom", item.id);
                if (item.theme)
                    localStorage.setItem("gb-theme", normalizeTheme(item.theme));
    ...
    const knownIds = new Set((catalog.items || []).map((r) => r.id));
    if (!knownIds.has(activeRomId))
        activeRomId = catalog.defaultId || "red";
    ...
    document.querySelectorAll("[data-rom]").forEach((el) => el.setAttribute("aria-pressed", String(el.dataset.rom === activeRomId)));
    const activeCatalogItem = (catalog.items || []).find((r) => r.id === activeRomId);
    ...
    gbCanvas.width = machine.width;
    gbCanvas.height = machine.height;
    ...
    const refreshAuto = () => {
        ...
        if (autoEl)
            autoEl.textContent = cp ? "checkpoint " + fmtAge(cp.savedAt) : "none yet";
    ...
    const writeAuto = () => {
        try {
            writeAutoCheckpoint(machine.saveKey(), machine.snapshot());
            refreshAuto();
        }
        catch (e) {
            console.warn("[save] auto-checkpoint failed", e);
        }
    };
    saver.onFlush = (bytes) => {
        if (battEl)
    ...
    const refreshSlots = () => {
        const states = listSaveStates(machine.saveKey(), NUM_STATE_SLOTS);
        const framesBySlot = new Map(states.map((s) => [s.slot, s.frames]));
        document.querySelectorAll("[data-state-load]").forEach((el) => {
            const slot = Number(el.dataset.stateLoad);
            const frames = framesBySlot.get(slot);
            el.disabled = frames === undefined;
            el.textContent = frames === undefined ? "Load\u00a0" + (slot + 1) : "Load\u00a0" + (slot + 1) + " (" + frames + "f)";
            el.title = frames === undefined ? "Empty save-state slot" : "Save-state for " + machine.saveKey() + " at frame " + frames;
        });
        document.querySelectorAll("[data-state-save]").forEach((el) => {
            const slot = Number(el.dataset.stateSave);
            if (el.classList.contains("confirm"))
                return;
            el.textContent = framesBySlot.has(slot) ? "Save\u00a0" + (slot + 1) + "*" : "Save\u00a0" + (slot + 1);
            el.title = "Save-state namespace: " + machine.saveKey();
        });
    };
    refreshSlots();
    const pendingOverwrite = { slot: -1, timer: null };
    const clearPending = () => {
        if (pendingOverwrite.timer)
            clearTimeout(pendingOverwrite.timer);
        const prev = pendingOverwrite.slot;
        pendingOverwrite.slot = -1;
        pendingOverwrite.timer = null;
        if (prev >= 0) {
            const btn = document.querySelector(`[data-state-save="${prev}"]`);
            if (btn) {
                btn.classList.remove("confirm");
                btn.textContent = "Save\u00a0" + (prev + 1);
            }
        }
    };
    const writeSlot = (slot) => {
        try {
            ...
            refreshSlots();
            toast("State saved to slot " + (slot + 1));
        }
        catch {
            toast("⚠ slot " + (slot + 1) + " save failed");
        }
    };
    const doSaveState = (slot) => {
        ...
        const btn = document.querySelector(`[data-state-save="${slot}"]`);
        if (btn) {
    ...
    const doLoadState = (slot) => {
        const st = readSaveState(machine.saveKey(), slot);
        if (!st) {
            toast("Slot " + (slot + 1) + " is empty");
            return;
        }
        machine.restore(st);
        toast("State loaded from slot " + (slot + 1));
    };
    document.getElementById("btn-save-now")?.addEventListener("click", async () => {
        await saver.flush();
        toast("Battery saved to this browser");
    });
    document.getElementById("btn-export-sav")?.addEventListener("click", async () => {
        await saver.flush();
        const name = (machine.saveKey().split(":")[1] || "game") + ".sav";
        const data = machine.getBatterySave();
        if (!data.length) {
            toast("No battery SRAM to export");
            return;
        }
        downloadBytes(name, data);
        toast("Exported " + name + " (" + (data.length / 1024).toFixed(0) + "KB)");
    });
    document.getElementById("btn-import-sav")?.addEventListener("click", async () => {
        ...
        if (!data)
            return;
        machine.loadBatterySave(data);
        await saver.flush();
        toast("Imported .sav (" + (data.length / 1024).toFixed(0) + "KB)");
    });
    document.getElementById("btn-export-states")?.addEventListener("click", () => {
        const bundle = exportSaveStateBundle(machine.saveKey(), NUM_STATE_SLOTS);
        if (!bundle.slots.length) {
            toast("No save states to export");
            return;
        }
        const name = (machine.saveKey().split(":")[1] || "game") + ".states.json";
        downloadJson(name, bundle);
        toast("Exported " + bundle.slots.length + " save state(s)");
    });
    document.getElementById("btn-import-states")?.addEventListener("click", async () => {
        const data = await pickFile(".json,application/json");
        if (!data)
            return;
        try {
            const bundle = JSON.parse(new TextDecoder().decode(data));
            const n = importSaveStateBundle(machine.saveKey(), bundle);
            refreshSlots();
            toast("Imported " + n + " save state(s)");
        }
        catch (e) {
            toast("Save-state import failed");
            console.warn(e);
        }
    });
    document.getElementById("btn-load-auto")?.addEventListener("click", () => {
        const cp = readAutoCheckpoint(machine.saveKey());
        if (!cp) {
            toast("No auto checkpoint yet");
            return;
        }
        machine.restore(cp.state);
        toast("Loaded auto checkpoint from " + fmtAge(cp.savedAt));
    });
    document.querySelectorAll("[data-state-save]").forEach((el) => el.addEventListener("click", () => doSaveState(Number(el.dataset.stateSave))));
    document.querySelectorAll("[data-state-load]").forEach((el) => el.addEventListener("click", () => doLoadState(Number(el.dataset.stateLoad))));
    const STATE_KEYS = {
        F5: () => doSaveState(0), F6: () => doSaveState(1), F7: () => doSaveState(2),
        F8: () => doLoadState(0), F9: () => doLoadState(1), F10: () => doLoadState(2),
        KeyM: () => { const muted = audio.toggleMute(); const sb = document.getElementById("btn-sound"); if (sb && audio.isStarted)
            sb.textContent = muted ? "🔇 Sound off" : "🔊 Sound on"; toast(muted ? "Muted" : "Unmuted"); },
    };
    ...
    scene.add(key);
    const rimPink = new THREE.PointLight(PAL.rim1, 60, 30);
    rimPink.position.set(-6, 2, -4);
    scene.add(rimPink);
    const rimCyan = new THREE.PointLight(PAL.rim2, 50, 30);
    rimCyan.position.set(6, -1, -5);
    scene.add(rimCyan);
    // ---- ground (reflection-ish dark plane) ----
    const ground = new THREE.Mesh(new THREE.CircleGeometry(40, 64), new THREE.MeshStandardMaterial({ color: 0x0a0b12, roughness: 0.85, metalness: 0.2 }));
    ...
    const rounded = (w, h, d, r) => (typeof RoundedBoxGeometry !== "undefined")
        ? new RoundedBoxGeometry(w, h, d, 4, r)
        : new THREE.BoxGeometry(w, h, d);
    const matte = (color, rough = 0.72, metal = 0.05) => new THREE.MeshStandardMaterial({ color, roughness: rough, metalness: metal });
    // body
    ...
    const body = new THREE.Mesh(new THREE.ExtrudeGeometry(shape, { depth: bd, bevelEnabled: true, bevelThickness: 0.08, bevelSize: 0.08, bevelSegments: 3, curveSegments: 12 }), matte(SHELL()));
    ...
    // IC helper
    const addChip = (x, y, w, h, labelHint = false) => {
    ...
    const addTrace = (x, y, w, h, z = 0.135) => {
    ...
    addTrace(-0.05, 0.92, 1.85, 0.025);
    addTrace(-0.98, 0.35, 0.025, 1.12);
    addTrace(0.10, -0.05, 1.55, 0.025);
    addTrace(1.02, -0.46, 0.025, 1.15);
    addTrace(-0.05, -1.55, 1.95, 0.025);
    addTrace(-1.02, -1.02, 0.025, 1.06);
    addTrace(0.72, 0.76, 0.025, 0.64);
    addTrace(0.42, -1.38, 0.025, 0.74);
    ...
    addTrace(-0.62, 1.35, 1.35, 0.022, -0.135);
    addTrace(-1.18, 0.75, 0.022, 1.10, -0.135);
    addTrace(0.72, 1.22, 0.022, 0.88, -0.135);
    addTrace(0.22, 0.22, 1.72, 0.022, -0.135);
    addTrace(-0.72, -0.42, 0.022, 1.08, -0.135);
    addTrace(0.52, -1.18, 1.30, 0.022, -0.135);
    addTrace(1.18, -1.55, 0.022, 0.78, -0.135);
    addTrace(-0.18, -1.92, 1.78, 0.022, -0.135);
    ...
    screen.position.set(0, 1.66, 0.69);
    gb.add(screen);
    // faint glass glow
    const glass = new THREE.Mesh(new THREE.PlaneGeometry(sw + 0.1, sh + 0.1), new THREE.MeshBasicMaterial({ color: SCREEN_GREEN(), transparent: true, opacity: 0.06, blending: THREE.AdditiveBlending }));
    glass.position.set(0, 1.66, 0.71);
    gb.add(glass);
    // ===========================================================================
    // BUTTONS — each is a clickable mesh that calls setButton(name, down)
    // ===========================================================================
    const pressables = [];
    function addPressable(mesh, btn) {
    ...
    const mkPad = (dx, dy, w, h, b) => {
        const m = new THREE.Mesh(new THREE.BoxGeometry(w, h, 0.3), new THREE.MeshStandardMaterial({ color: BTN_DARK(), roughness: 0.55, transparent: true, opacity: 0 }));
        m.position.set(dpadOrigin.x + dx, dpadOrigin.y + dy, dpadOrigin.z + 0.02);
        addPressable(m, b);
    };
    dpadV.position.copy(dpadOrigin);
    dpadH.position.copy(dpadOrigin);
    ...
    const setCol = (mat, hex) => { if (mat?.color)
    ...
    window.__gbApplyTheme = applyTheme;
    ...
    const pickButton = (clientX, clientY) => {
        ...
        return hits.length ? hits[0].object.userData.btn : null;
    };
    ...
    const releasePtr = () => { if (activePtrBtn) {
        press(activePtrBtn, false);
        activePtrBtn = null;
    ...
    const validSpeed = (v) => ([1, 2, 4].includes(v) ? v : 2);
    ...
    function applyPlaybackSpeed(v) {
        ...
        if (speedStatus)
            speedStatus.textContent = PLAYBACK_SPEED + "x gameplay";
