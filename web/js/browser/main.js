...
import { BatteryAutoSaver, writeSaveState, readSaveState, listSaveStates, writeAutoCheckpoint, readAutoCheckpoint, downloadBytes, pickFile, } from "./save.js";
...
async function boot() {
    ...
    const writeSlot = (slot) => {
        try {
            ...
            toast("⚠ slot " + (slot + 1) + " save failed (storage full?)");
        }
    };
    ...
    document.getElementById("btn-export-sav").addEventListener("click", () => {
        ...
        downloadBytes(name, machine.getBatterySave());
        toast("Exported " + name);
    });
    ...
    const STATE_KEYS = {
        ...
        KeyM: () => { const muted = audio.toggleMute(); if (soundBtn && audio.isStarted)
            soundBtn.textContent = muted ? "🔇 Sound off" : "🔊 Sound on"; toast(muted ? "Muted" : "Unmuted"); },
    };
    ...
    const startAudio = async () => {
        try {
            ...
            console.warn("[gb] audio start failed", e);
        }
    };
    ...
    document.querySelectorAll("[data-btn]").forEach((el) => {
        ...
        const down = (ev) => { ev.preventDefault(); machine.setButton(b, true); };
        const up = (ev) => { ev.preventDefault(); machine.setButton(b, false); };
    ...
    function loop(now) {
        ...
        if (frameCount === 1)
            console.log("[gb] first frame rendered");
