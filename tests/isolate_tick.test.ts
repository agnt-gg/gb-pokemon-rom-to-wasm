...
async function runMode(mode: "none"|"ppu"|"timer"|"both"): Promise<string> {
  const mmu = new MMU(new Uint8Array(readFileSync(romPath)));
  ...
  const imports={env:{mem:memory,rb:(a:number)=>mmu.read(a&0xffff),wb:(a:number,v:number)=>mmu.write(a&0xffff,v&0xff),
    tick:(c:number)=>{cpu.cycles+=c; if(mode==="ppu"||mode==="both")ppu.step(c); if(mode==="timer"||mode==="both")timer.step(c);},
  ...
  while(n++<300000){
    ...
    if(pc===0x0038) return `mode=${mode}: CRASH at dispatch ${n}`;
    const ret=ex.run(pc);
    if(ret===UNKNOWN_BLOCK){w2c();cpu.step();c2w();}
    else if(ret===SENTINEL_HALT)cpu.halted=true;
  }
  return `mode=${mode}: OK (300k dispatches, LCDC=0x${mmu.rawIoRead(0xff40).toString(16)})`;
}

for (const m of ["none","ppu","timer","both"] as const) console.log(await runMode(m));
