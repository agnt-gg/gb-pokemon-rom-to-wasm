/** Run BrowserMachine to ~frame 13, then dump HRAM 0xFF80-0xFFFE and disasm from 0xFF8E. */
...
let firstHram=-1, dispatches=0;
function rc(target:number){const start=cpu.cycles;let g=0;while(cpu.cycles-start<target&&g++<200000){
  w2c();const ic=cpu.serviceInterrupts();if(ic>0){c2w();tk(ic);} if(cpu.halted){cpu.cycles+=4;tk(4);if(cpu.serviceInterrupts()>0){cpu.halted=false;c2w();}continue;}
  const cur=ex.get_PC()&0xffff;
  if(cur>=0xff80&&cur<=0xfffe&&firstHram<0){firstHram=cur;
    console.log(`First HRAM exec at 0x${cur.toString(16)} (dispatch ${dispatches}). HRAM dump:`);
    ...
    console.log("disasm from 0x"+cur.toString(16)+":");
    let a=cur;for(let i=0;i<24;i++){const ins=decode(new Uint8Array([mmu.read(a),mmu.read(a+1),mmu.read(a+2)]),0,a);console.log(`    0x${a.toString(16)}: ${ins.text} [${mmu.read(a).toString(16)}] term=${ins.isTerminator}`);if(ins.isTerminator)break;a+=ins.length;}
...
try{for(let f=0;f<80;f++)rc(70224);}catch(e){}
