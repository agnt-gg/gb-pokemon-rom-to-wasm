...
while(n++<200000){
  ...
  if(pc===0x0038){console.log("CRASH. Last 30 blocks:");for(const t of trail)console.log("  "+t);break;}
  // disasm block head
  const ins=decode(new Uint8Array([mmu.read(pc),mmu.read(pc+1),mmu.read(pc+2)]),0,pc);
  trail.push(`0x${pc.toString(16).padStart(4,"0")} SP=0x${sp.toString(16)} LY=${mmu.rawIoRead(0xff44)} LCDC=0x${mmu.rawIoRead(0xff40).toString(16)} | ${ins.text}`);
